...
class SketchSvm:
    ...
    def __init__(self):
        ...
        self.write_model = 'models/' + self.timestamp + '.sav'
    ...
    def load_images(self, train_path):
        ...
        for cat in self.categories:
            ...
            images_in_cat = glob.glob(cat + '/*150x150.png')
    ...
    def train(self, google, c_range, gamma_range, kernel):
        ...
        # 3. We use scikit-learn to train a SVM classifier. Specifically we use a GridSearchCV in our case
        # linear?
        clf = GridSearchCV(svm.SVC(kernel=kernel), param_grid=[
            {'C': c_range, 'gamma': gamma_range}
        ])
        clf.fit(x_train, y_train)

        # save model
        pickle.dump(clf, open(self.write_model, 'wb'))
