...
class CNNModel:
    ...
    def load_model(classes=10):
        ...
        model.add(Convolution2D(input_shape=input_shape, data_format='channels_last', strides=(3, 3), filters=64,
                                kernel_size=(15, 15), padding="same", activation="relu"))
        # Inout 71x71 Output: 35x35
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same"))

        # 2 Input: 35x35 Output: 31x31
        model.add(Convolution2D(kernel_size=(5, 5), filters=128, strides=(1, 1), activation="relu", padding="same"))
        # Input: 31x31 Output: 15x15
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same"))
        ...
        model.add(Convolution2D(kernel_size=(1, 1), filters=250, strides=(1, 1), activation="relu", padding="same"))
