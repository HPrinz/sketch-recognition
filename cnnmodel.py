...
class CNNModel:
    ...
    def load_model(classes=10, img_rows=28, img_cols=28):
        ...
        model.add(Convolution2D(kernel_size=(3, 3), filters=64, strides=(1, 1), activation="relu", padding="same"))
        # Input: 31x31 Output: 15x15s
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same"))

        # Input: 15x15 Output: 15x15
        model.add(Convolution2D(kernel_size=(3, 3), filters=128, strides=(1, 1), activation="relu", padding="valid"))
        model.add(Convolution2D(kernel_size=(3, 3), filters=128, strides=(1, 1), activation="relu", padding="valid"))
        model.add(Convolution2D(kernel_size=(3, 3), filters=128, strides=(1, 1), activation="relu", padding="valid"))
        ...
        model.add(Convolution2D(kernel_size=(7, 7), filters=256, strides=(1, 1), activation="relu", padding="same"))
        # Input: 1x1 Output: 1x1
        model.add(Dropout(rate=0.5))

        model.add(Convolution2D(kernel_size=(1, 1), filters=256, strides=(1, 1), activation="relu", padding="same"))
        model.add(Dropout(rate=0.5))
